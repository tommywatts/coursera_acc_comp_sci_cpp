...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
 ...
 std::vector< TreeNode* > childPtrs_below;
  
  // Initialize childPtrs
 childPtrs = rootNodePtr->childrenPtrs;
  
// while child poiters exist
 while( childPtrs.size() != 0) {
    childPtrs_below.clear();
    
    for( std::size_t i = 0 ; i <  childPtrs.size() ; i++ ) {
      // push child data into results

      T data = childPtrs[i]->data;
      results.push_back( data );
      
      // search the nodes below the childPtr, and push into childPtrs_below
      for( std::size_t j = 0 ; j < childPtrs[i]->childrenPtrs.size() ; j ++ ) {
        childPtrs_below.push_back( childPtrs[i]->childrenPtrs[j] );
      }
    }
    childPtrs.clear();

    // start search again from the childPtrs belw the child node
    childPtrs = childPtrs_below;
  }
